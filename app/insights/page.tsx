  ...
  return (
    <div>
      ...
      <header className="mb-8">
        <h1 className="text-3xl font-bold text-stone-900 mb-2">Property Tax Insights</h1>
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        {articles.map((a) => (
          <Link
            ...
            className="block rounded-xl border border-stone-200 hover:border-amber-500 hover:bg-amber-50 p-5 transition-colors"
          >
            <div className="text-xs text-stone-400 mb-1">
              <time dateTime={a.date}>{a.date}</time>
            </div>
            <h2 className="text-lg font-bold text-stone-900 mb-2">{a.title}</h2>
            <p className="text-sm text-stone-600">{a.summary}</p>
          </Link>
        ))}
      </div>

      <section className="mt-12 p-6 rounded-xl bg-stone-50 border border-stone-200">
        <h2 className="text-xl font-bold text-stone-900 mb-3">Run the numbers yourself</h2>
        <p className="text-sm text-stone-600 mb-4">
          These insights are based on aggregate data. Get the specific numbers for your situation.
        </p>
        <ul className="space-y-2 text-sm">
          <li>
            <Link href="/calculator/" className="text-amber-800 hover:underline font-medium">Property tax calculator</Link>
            <span className="text-stone-500"> — estimate your annual bill by state</span>
          </li>
          <li>
            <Link href="/compare/" className="text-amber-800 hover:underline font-medium">Compare any two states</Link>
            <span className="text-stone-500"> — side-by-side rates and median bills</span>
          </li>
        </ul>
      </section>
    </div>
